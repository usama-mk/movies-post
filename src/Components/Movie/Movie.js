...
function Movie({ movie, selectMovie, key, right, left, mid, selectedMovie }) {
  ...
  const selectedLeft = useSelector(state => state.selectedLeft);
  ...
  const ddLeft = useSelector((state) => state.ddLeft);
  ...
  const getMoviesF2 = async (first, last) => {
    if (first || last) {
      ...
      const filterArray = moviesArray.filter((movie) => {
        if (
          movie.title.replace(/ .*/, "") == first ||
          getLastWord(movie.title) == last
        ) {
          return movie;
        }
      });
  ...
  const getMoviesLeft = async (first) => {
    if (first) {
      ...
      const moviesObj = await moviesRes.json();
      const moviesArray = moviesObj.results;
      const filterArray = moviesArray.filter((movie, key) => {
      ...
      // dispatch(setSelectedLeft(filterArray))
      // console.log(`select left movie: ${ddLeft[0]}`)
     
      filterArray.map((object,key)=>{
        if(key==0){
          dispatch(setSelectedLeft(object))
          return
        }
      })
  ...
  const getMoviesRight = async (last) => {
    if (last) {
      ...
      const filterArray = moviesArray.filter((movie) => {
        console.log(
          `we are searching::  ${last.toLowerCase()}  ::for right and matching with ${movie.title
            .replace(/ .*/, "")
            .toLowerCase()}`
        );
        if (
          movie.title.replace(/ .*/, "").toLowerCase() == last.toLowerCase()
        ) {
  ...
  const setDdOptionLeft=(id)=>{
    dispatch(setSelectedLeft(JSON.parse(document.getElementById(id).value)))
  ...
  return (
    <div className="movie">
      <div className="selectWrapper">
        ...
        {    ddMoviesLeft.length > 0 &&
            ddMoviesLeft.map((movie, key) => {
              return (
      ...
      {left && (
        <div>
          <img
            src={selectedLeft ? IMG_API + selectedLeft?.poster_path : ph}
            alt={selectedLeft?.title}
          />
          <div className="movie info">
            <h3>{selectedLeft?.title}</h3>
            <h4>Ratings: {selectedLeft?.vote_average}</h4>

            <div className="overview">{selectedLeft?.overview}</div>
          </div>
        </div>
      )}
      {mid && (
        <div>
          <img
            src={selectedMovie ? IMG_API + selectedMovie?.poster_path : ph}
            alt={titleOpt}
          />
          <div className="movie info">
