...
function Movies() {
  ...
  const postData = async () => {
    const title = ` ${dl[0]?.title ? dl[0]?.title : ""} ${
      selectedMovie && selectedMovie?.title
    } ${dr[0]?.title ? dr[0]?.title : ""}`;
    const postObj = {
      MovieTitle: title,
      PostTitle: interest,
      Synopsis: synopsis,
    ...
    <div className="moviesWrapper">
              ...
              return (
                <div>
                  <Movie
                    ...
                    left={key == 0 ? true : false}
                    mid={key == 1 ? true : false}
      ...
      {movies.length > 0 && (
        <div className="moviesData">
          <h1 className="center">{` ${selectedLeft?.title ? selectedLeft?.title : ""} ${
            selectedMovie && selectedMovie?.title
          } ${dr[0]?.title ? dr[0]?.title : ""}`}</h1>
          <h5>
